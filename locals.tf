locals {
  ...
  cifs_creds_file_path    = format("/etc/smbcredentials/%s.cred", module.storage_account.name)
  cifs_creds_file_content = <<-EOF
    username=${module.storage_account.name}
    password=${module.storage_account.resource.primary_access_key}
EOF
  mount_options = {
    for s in var.file_shares : s.name => s.protocol_enabled == "NFS" ?
  ...
  mount_endpoints = { for s in var.file_shares :
    s.name => s.protocol_enabled == "NFS" ?
    format("%s:/%s/%s", module.storage_account.resource.primary_file_host, module.storage_account.name, s.name) :
    format("//%s/%s", module.storage_account.resource.primary_file_host, s.name)
  }

  mount_paths = { for s in var.file_shares : s.name => "/mnt/${module.storage_account.name}/${s.name}" }
